...
import { parseImageEntries } from "../logic/imageEntryUtils";
...
function buildCommentDisplayList(comments) {
  ...
  return [...safeList]
    ...
    .map((item) => ({
      ...
      _imageEntries: parseImageEntries(item.image_url),
    }));
}

function CommentImageList({ imageEntries }) {
  ...
  return (
    <div style={{ padding: "10px 0", borderBottom: "1px dashed #dcece4" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        <img
          src={getAvatarWithFallback(item.avatar_url, item.user_phone, item.username)}
          alt="readonly-avatar"
          {...buildImageLoadingProps()}
          style={{ width: "24px", height: "24px", borderRadius: "50%", objectFit: "cover", border: "1px solid #d3e6dc" }}
        />
      ...
      <CommentImageList imageEntries={item._imageEntries || []} />
...
function UserRecommendedPlaceReadonly({ place, visible }) {
  ...
  const placeImageEntries = useMemo(() => parseImageEntries(place?.images || place?.image_url || []), [place?.image_url, place?.images]);
  ...
  useEffect(() => {
    ...
    fetch(`${authApiBase}/api/comments/rec_${place.id}`, { signal: controller.signal })
      ...
      .catch((fetchError) => {
        ...
        console.error("加载推荐评论失败:", fetchError.message);
        setError(`评论加载失败：${fetchError.message}`);
      })
      .finally(() => {
  ...
  return (
    <div style={{ marginTop: "10px", borderRadius: "16px", border: "1px solid #d3eadf", background: "rgba(255,255,255,0.82)", padding: "10px" }}>
      ...
      <div style={{ marginTop: "8px", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
        ...
        <span style={{ borderRadius: "999px", border: "1px solid #d6e8de", background: "#f8fffb", padding: "4px 9px", fontSize: "12px", color: "#406957", fontWeight: 700 }}>评论 {totalCommentCount}</span>
